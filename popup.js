...
function saveScheduleData(data) {
  ...
  // Sauvegarder également dans chrome.storage.local pour plus de fiabilité
  chrome.storage.local.set({
    ...
    console.log('Données sauvegardées dans chrome.storage.local');
  });
}
...
function loadScheduleData() {
  ...
  chrome.storage.local.get(['schedule', 'uniformEndTime', 'workHours', 'startTime'], function(localData) {
    if (localData.schedule) {
      console.log('Données chargées depuis chrome.storage.local');
      ...
      chrome.storage.sync.get(['schedule', 'uniformEndTime', 'workHours', 'startTime'], function(syncData) {
        if (syncData.schedule) {
          console.log('Données chargées depuis chrome.storage.sync');
...
function clearScheduleData() {
  // Effacer les données de chrome.storage.sync
  chrome.storage.sync.remove(['schedule', 'uniformEndTime', 'workHours', 'startTime'], function() {
    console.log('Données effacées de chrome.storage.sync');
  });
  
  // Effacer les données de chrome.storage.local
  chrome.storage.local.remove(['schedule', 'uniformEndTime', 'workHours', 'startTime'], function() {
    console.log('Données effacées de chrome.storage.local');
  });
